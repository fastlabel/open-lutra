...
class TestGetActiveDestination:
    def test_returns_disabled_destination_when_unset(self) -> None:
        destination = get_active_destination(_settings())
        assert isinstance(destination, DisabledDestination)
    ...
    def test_conforms_to_protocol(self) -> None:
        ...
        assert isinstance(
            get_active_destination(_settings(upload_destination="s3")),
            UploadDestination,
        )
        assert isinstance(
